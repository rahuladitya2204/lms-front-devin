...
import CoinImage from "@Learner/Screens/Account/LearnerWallet/CoinImage";
...
export default function LearnerHeader({ children }: LearnerHeaderProps) {
  ...
  const WalletButton = loadingLearnerDetails ? (
    HeaderButtonSkeleton
  ) : (
    <NavLink
      to={`../app/wallet`}
      children={({ isActive }) => {
        return loadingLearnerDetails ? (
          HeaderButtonSkeleton
        ) : (
          <Tooltip
            title={
