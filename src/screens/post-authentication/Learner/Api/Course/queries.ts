...
export const useGetEnrolledCourses = () => {
    const { data = [], isFetching: isLoading } =
      useQuery<Types.EnrolledCourseDetails[]>([KEYS.GET_ENROLLED_COURSES], () => GetLearnerEnrolledCourses().then((enrolledCourses => {
  ...
  export const useGetCourseQuestions = (courseId:string) => {
    const { data = [], isFetching: isLoading } =
      useQuery<Types.CourseQuestion[]>([KEYS.GET_COURSE_QUESTIONS,courseId], ()=>GetCourseQuestions(courseId))
    return {
      data,
      isLoading
    }
  ...
  export const useGetCoursesOfOrganisation = () => {
    const { data = [], isFetching: isLoading } =
      useQuery<Types.Course[]>([KEYS.GET_COURSES_OF_ORGANISATION], ()=>getCoursesOfOrganisation())
    return {
      data,
      isLoading
    }
