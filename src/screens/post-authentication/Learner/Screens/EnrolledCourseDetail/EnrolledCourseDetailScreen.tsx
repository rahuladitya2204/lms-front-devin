...
const EnrolledCourseDetailScreen: React.FC<EnrolledCourseDetailScreenPropsI> = (
  ...
  const continueLearning = () => {
    navigate(`/app/course/${lastPlayed.course}/player/${lastPlayed.item}`);
  };

  const playItem = (itemId: string) => {
    navigate(`/app/course/${courseId}/player/${itemId}`);
  };

  const { isMobile, isTablet } = useBreakpoint();
  const defaultLanguage = 'eng'
  return (loadingCourse || loadingEp) ? <Row gutter={[20, 30]}>
    <Col span={24}>
      <Skeleton.Button active block style={{ height: 150 }} /></Col>
    <Col span={24}>
      <Skeleton.Button active block style={{ height: 300 }} /></Col>
  </Row>
    : (
      <Row>
        <Col span={24}>
            ...
            <Col span={24}>
              <BackButton onClick={() => {
                if (enrolledPackageId) {
                  navigate(`/app/${enrolledPackageId}/enrolled-package`)
                }
              }}></BackButton>
            </Col>
            <Col span={24}>
