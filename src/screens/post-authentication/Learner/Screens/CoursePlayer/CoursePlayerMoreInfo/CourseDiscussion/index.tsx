  ...
  return (
    <Row>    
      <Col span={24}>
        ...
        {!question._id ?
          <CourseQuestionsList selectQuestion={setQuestion} course={props.course} /> :
          <Card title={<Text strong>{question.title}</Text>}
            extra={question._id ?
            [<Button onClick={() => setQuestion(Constants.INITIAL_QUESTION_DETAILS)}>Back</Button>] : []}
            bordered={false}>
            <div dangerouslySetInnerHTML={{ __html: question.description }}></div>
            <CourseQuestionAnswers courseId={props.course._id} questionId={question._id} />
          </Card>}
