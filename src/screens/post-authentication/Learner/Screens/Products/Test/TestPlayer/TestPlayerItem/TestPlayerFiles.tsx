      ...
      <Card title="Answer Images" extra={[!props.review ? UploadButton : null]}>
        {imageIssues && imageIssues?.length ? (
          <Alert
            ...
            type="error"
            message={<Text strong>Issue with images uploaded</Text>}
            description={
              <Row>
                <Col span={24}>
        ...
        {!files.length ? (
          <Empty
            description={
              <Row gutter={[10, 20]}>
                <Col span={24}>
                  <Text>Upload your handwritten answer images here</Text>
                </Col>
...
const TestPlayerFileItem: React.FC<{ fileId: string; fileUrl: string }> = ({
  fileId,
  fileUrl,
}) => {
  const { currentQuestion } = useReviewQuestion();
