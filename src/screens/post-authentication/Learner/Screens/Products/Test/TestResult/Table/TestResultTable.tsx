...
const TestResultTable: React.FC = () => {
  ...
  const onExpand = (
    expanded: boolean,
    record: Types.TestStatusQuestionStats
  ) => {
            ...
            <Card title={section.title}>
                      ...
                      <Row gutter={[20, 20]}>
                        <Col span={24}>
                          {record?.feedback?.met ? (
                            <Alert
                              type="success"
                              message="What was good"
                              description={record.feedback.met.text}
                            />
                          ) : null}
                        </Col>
                        <Col span={24}>
                          {record?.feedback?.notMet ? (
                            <Alert
                              type="error"
                              message="What could be improved"
                              description={record.feedback.notMet.text}
                            />
                          ) : null}
                        </Col>
                      </Row>
              ...
              >
                ...
                <Table.Column
                  ...
                  render={(_, record: TestResultItem) =>
                    record.type === "subjective" ? (
                      <Tag color="blue-inverse">Subjective</Tag>
                    ) : (
                      <Tag color="orange-inverse">
                        {record?.type?.toUpperCase()} CHOICE
                      </Tag>
                    )
                  }
                />
                ...
                <Table.Column
                  ...
                  key="timeSpent"
                  render={(timeSpent) => (
                    <span>{timeSpent ? Utils.formatTime(timeSpent) : "-"}</span>
                  )}
                />
                <Table.Column
                  title="Score"
                  ...
                  key="scoreAchieved"
                  render={(scoreAchieved) => (
                    <span>{scoreAchieved >= 0 ? scoreAchieved : "-"} </span>
                  )}
                />
              </Table>
            </Card>
    ...
    <Space>
      ...
      {item.type === "subjective" ? (
        <>
          {/* @ts-ignore */}
          {item.notEvaluated ? (
            <Tag color="orange-inverse">Not evaluated</Tag>
          ) : item.isAnswered ? (
            <Tag>Attempted</Tag>
          ) : (
            <Tag>Not Attempted</Tag>
          )}
        </>
      ) : (
        <>
