...
const CreateTicket: React.FC<CreateTicketComponentPropsI> = (props) => {
  ...
  const { mutate: createTicket, isLoading: createTicketLoading } =
    Learner.Queries.useCreateTicket();

  const { data: categories } = Learner.Queries.useGetTicketCategories();
  ...
  const subCategories = categories
    .filter((cat) => category && cat.parentCategory === category)
    .map((c) => {
      return {
        label: c.title,
        value: c._id,
      ...
      <Form form={form} onFinish={onSubmit} layout="vertical">
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please mention subject of the ticket!",
            },
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please add some details about the issue",
            },
            {
              min: 5,
        ...
        >
          <TextArea
            height={200}
            placeholder="Please enter email of the Ticket"
          />
        </Form.Item>
