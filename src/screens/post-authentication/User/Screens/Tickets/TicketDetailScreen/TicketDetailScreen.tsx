...
export default function TicketDetail() {
  const { id } = useParams();
  const { data: ticket, isLoading: loading } = User.Queries.useGetTicketDetails(
    id + ""
  );
  ...
  const { mutate: replyToTicket, isLoading: postingReply } =
    User.Queries.useReplyToTicket();
  ...
  return (
    <Header hideBack title={<Text>Support Ticket: {ticket.id}</Text>}>
      {loading ? (
        <>
          <Card>
            <Skeleton />
            ...
            {ticket.replies.length ? (
              <>
                <List
                  ...
                  header={
                    <Title level={5}> {ticket.replies.length} Replies</Title>
                  }
                    ...
                    return (
                      <li>
                        <Comment
                          ...
                          author={
                            <Text strong>
                              {isFromSupportAgent ? "Me" : createdBy?.name}
                            </Text>
                          }
                          avatar={
                            <Avatar
                          ...
                          content={<HtmlViewer content={item.message} />}
                          datetime={
