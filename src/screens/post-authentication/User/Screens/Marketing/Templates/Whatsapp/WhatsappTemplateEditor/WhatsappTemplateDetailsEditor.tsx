...
const WhatsappTemplateDetailsEditor: React.FC<
  CreateWhatsappTemplateComponentPropsI
> = (props) => {
  const { id } = useParams();
  const templateId = props.id || id;
  const { data: template } =
    User.Queries.useGetWhatsappTemplateDetails(templateId);
  const { useWatch } = Form;
  ...
  const {
    data: { WhatsappTemplatesMap },
  } = Common.Queries.useGetAppConfig("user");
  const MailType = WhatsappTemplatesMap[template.whatsappType]
    ? WhatsappTemplatesMap[template.whatsappType]
    : {};
    ...
    <Fragment>
      ...
      <Space direction="vertical" size={[30, 30]} style={{ width: "100%" }}>
        ...
        <Form.Item name="content" label="Body of the whatsapp" required>
          <TextArea html variables={variables} name={["content"]} />
        </Form.Item>
      </Space>
    </Fragment>
  );
