...
export default function OrgProfile() {
  ...
  const { mutate: updateOrgAccount, isLoading: loading } =
  ...
  const onSubmit = (data: Partial<Types.OrganisationSetting>) => {
    ...
    if (data.branding?.colors.primary?.metaColor) {
      ...
      data.branding.colors.primary =
        data.branding?.colors.primary?.metaColor?.originalInput;
    }
    // @ts-ignore
    if (data.branding?.colors.secondary?.metaColor) {
      // @ts-ignore
      data.branding.colors.secondary =
        data.branding?.colors.secondary?.metaColor?.originalInput;
    }
    updateOrgAccount({ data });
  };
  const { isFontValidating, isFontValid, validateFontName } =
    useFontValidation(form);
        ...
        <Form.Item name="email" required label="Email">
            ...
            <Col span={6}>
              ...
              >
                <Select
                  mode="tags"
                  style={{ width: "100%" }}
                  placeholder="Select gradient type 1"
                />
              </Form.Item>
            </Col>
            <Col span={6}>
              ...
              >
                <Select
                  mode="tags"
                  style={{ width: "100%" }}
                  placeholder="Select gradient type 2"
                />
              </Form.Item>
            </Col>
            <Col span={6}>
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please enter a description for your organisation",
              },
            ]}
          >
            <Input
          ...
          <Form.Item
            name={["branding", "font", "url"]}
            required
            label="Font Link"
          >
            <Input
              style={{ width: 500 }}
              placeholder="Please enter font name which you want to be used"
            />
          </Form.Item>
