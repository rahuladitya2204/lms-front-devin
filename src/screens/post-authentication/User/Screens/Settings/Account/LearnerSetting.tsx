...
export default function LearnerSetting(props: LearnerSettingPropsI) {
  ...
  const { mutate: updateOrgSetting, isLoading: updatingSetting } =
  ...
  const handleDeleteBanner = (key: string) => {
    ...
    const updatedBanners = banners.filter(
      (banner: Types.BannerSetting) => banner._id !== key
    );
      ...
      <Row gutter={[20, 30]}>
        <Col span={24}>
          ...
          >
            ...
            <Row gutter={[16, 16]}>
              {form
                .getFieldValue("banners")
                ?.map((banner: Types.BannerSetting, index: number) => (
                          ...
                          <Table
                            ...
                            columns={[
                              ...
                              },
                              {
                                ...
                                dataIndex: "display",
                                key: "display",
                                render: (display: { from: Date; to: Date }) =>
                                  `${dayjs(display?.from).format(
                                    "LL"
                                  )} - ${dayjs(display?.to).format("LL")}`,
                              },
                              {
                                title: "CTAs",
                                dataIndex: "ctas",
          ...
          <Card title="Authentication">
            <Form.Item name={["register", "type"]}>
              <Select placeholder="Select Register Strategy">
                <Select.Option
                  key={Enum.LearnerRegisterType.LAZY}
                  value={Enum.LearnerRegisterType.LAZY}
                >
                  Lazy
                </Select.Option>
                <Select.Option
                  key={Enum.LearnerRegisterType.STRICT}
                  value={Enum.LearnerRegisterType.STRICT}
                >
                  Strict
                </Select.Option>
                <Select.Option
                  key={Enum.LearnerRegisterType.INVITE_ONLY}
