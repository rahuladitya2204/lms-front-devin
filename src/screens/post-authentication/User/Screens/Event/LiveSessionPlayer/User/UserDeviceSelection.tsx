import { Button, Card, Checkbox, Result } from "@Lib/index";
import {
  CameraSelection,
  PreviewVideo,
  QualitySelection,
...
const UserDeviceSelection = () => {
  ...
  const { mutate: startSession, isLoading: startinSession } =
    User.Queries.useStartEvent();

  const { mutate: endMeeting, isLoading: endingSession } =
    User.Queries.useEndMeeting();

  const startMeeting = () => {
    startSession(
      { session: eventId + "", enableRecording: recordMeeting },
      {
        onSuccess: (session) => {
  ...
  useEffect(() => {
    // @ts-ignore
    start(session, {
      metadata: Constants.INITIAL_EVENT_ATTENDEE_DETAILS.metadata,
    });
  }, []);
    ...
    <>
      {session.startedAt && session.endedAt ? (
        ...
        >
          ...
          >
            {" "}
            <VideoCameraOutlined /> Record Meeting
          </Checkbox>
          {session.startedAt && !session.endedAt ? (
            <Button
              style={{ margin: "10px 0" }}
              type="primary"
              onClick={() => navigate(`${session.metadata.MeetingId}/session`)}
              block
            >
              Join Meeting
            </Button>
          ...
          {session.startedAt && !session.endedAt ? (
            <Button
              ...
              style={{ margin: "10px 0" }}
              onClick={() => endMeeting({ session: eventId + "" })}
              block
            >
              End Meeting
            </Button>
