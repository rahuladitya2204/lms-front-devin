...
const CreateEvent: React.FC<CreateEventComponentPropsI> = (props) => {
  ...
  const { mutate: createEvent, isLoading: createEventLoading } =
    User.Queries.useCreateEvent();
  const { mutate: updateEvent, isLoading: updateEventLoading } =
    User.Queries.useUpdateEvent();

  const { data: eventDetails, isFetching: loadingEvent } =
    User.Queries.useGetEventDetails(eventId + "", {
      enabled: !!eventId,
    });
  ...
  const { mutate: unpublishEvent, isLoading: unpublishingEvent } =
    User.Queries.useUnpublishEvent();
  const UnpublishButton = (
    <Button
      type="primary"
      loading={unpublishingEvent}
      onClick={() => {
        confirm({
          title: "Are you sure?",
  ...
  const PublishEvent = (
    <Button
      ...
      onClick={() => {
        form
          .validateFields() // Trigger validation manually
          .then(() => {
  ...
  return (
    <Header
      showBack
      title={isUpdate ? `${eventDetails.title}` : `Create Event`}
      extra={
        ...
        <Row>
          <Col span={24}>
                  ...
                  <Col span={16}>
                    <Form.Item
                      rules={[
                        {
                          required: true,
                          message: "Please enter a title of the events",
                        },
                      ]}
                      name="title"
                      label="Event Title"
                          ...
                          rules={[
                            {
                              required: true,
                              message:
                                "Please enter start time for the livestream",
                            },
                          ]}
                          name="scheduledAt"
                          label="Scheduled For"
                      ...
                      <Select
                        options={[
                          { label: "Webinar", value: Enum.EventType.WEBINAR },
                          {
                            ...
                            value: Enum.EventType.CONVERSATIONAL,
                          },
            ...
            <Card
              title="Outcomes"
              extra={
                <ActionModal cta={<Button type="primary">Add Outcome</Button>}>
                  <AddOutcome
                    submit={(e: Outcome) => {
                      console.log(e, "e");
