import {
  Alert,
  Button,
  Card,
  Col,
  Row,
  Statistic,
  Table,
  Tag,
  Typography
} from 'antd'
import { Enum, Types, User } from '@adewaskar/lms-common'

import { capitalize } from 'lodash'
import dayjs from 'dayjs'
import { useParams } from 'react-router'

const { Text } = Typography

const LiveTestAttendedList = () => {
  const { testId } = useParams()
  const { data } = User.Queries.useGetLiveTestResult(testId + '')
  const { data: liveTest } = User.Queries.useGetLiveTestDetails(testId + '')
  const TOTAL_POSSIBLE_SCORE = liveTest.sections.reduce((acc, section) => {
    const sectionScore = section.items.reduce(
      (sectionAcc, item) => sectionAcc + (item.score || 0),
      0
    )
    return acc + sectionScore
  }, 0)
  return (
    <Table dataSource={data}>
      <Table.Column
        title="Student Name"
        dataIndex="learnerName"
        key="learnerName"
      />
      <Table.Column
        title="Result"
        render={(_: any, record: Types.LiveTestLearnerResult) => (
          <Tag color={record.status === 'passed' ? 'green-inverse' : 'red-inverse'}>
            {capitalize(record.status)}
          </Tag>
        )}
        key="result"
      />
      <Table.Column
        title="Score"
        render={(_: any, record: Types.LiveTestLearnerResult) =>
          `${record.score} marks (${record.score /
            TOTAL_POSSIBLE_SCORE *
            100}%)`
        }
        key="result"
      />

      <Table.Column
        title="Time Spent"
        render={(_: any, record: Types.LiveTestLearnerResult) =>
          `${Math.ceil(record.timeSpent / 3600)} min`
        }
        key="result"
      />

      <Table.Column
        title="Submitted At"
        render={(_: any, record: Types.LiveTestLearnerResult) =>
          dayjs(record.submittedAt).format('LLL')
        }
        key="result"
      />
    </Table>
  )
}

export default LiveTestAttendedList
