...
function LiveTestLearners(props: LiveTestLearnersPropsI) {
  const { data, isLoading: loading } = User.Queries.useGetEnrolledProductLearners(
    props.liveTestId + ''
  )

  const {
    mutate: removeLearnerFromLiveTest
                ...
                <Button
                  onClick={() => {
                    confirm({
                      ...
                      content: `You want to refund this liveTest to the user`,
                      onOk() {
                        removeLearnerFromLiveTest({
                          courseId: props.liveTestId,
                          // @ts-ignore
